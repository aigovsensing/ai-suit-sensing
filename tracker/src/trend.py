...
[작성 지침]
1. 제목은 "## 🗓️ (조간뉴스: {today_kst}) {lookback_days}일간의 소송센싱 주요 동향 현황"으로 시작해주세요. (날짜 {today_kst} 는 반드시 그대로 포함)
...
[작성 지침]
1. 제목은 "## 🧠 (석간뉴스: {today_kst}) 당일 신규/업데이트 소송건 요약 보고서 (Gemini)"로 시작해주세요. (날짜 {today_kst} 는 반드시 그대로 포함)
