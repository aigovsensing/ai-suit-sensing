...
| `ENABLE_EMAIL_CONSOLIDATED` | **Variable** | `1` | **📑 당일 소송건들 통합 정리 리포트 이메일 발송 여부.** 기본 활성(`1`). `0` 설정 시 통합 정리 이메일만 발송을 중단합니다. KST 23시(KST 23:00) 별도 워크플로([`consolidated-email.yml`](../.github/workflows/consolidated-email.yml))로 발송됩니다. |
| `MORNING_REPORT_HOUR` | **Variable** | `7` | **🗓️ 조간뉴스 발행 기준 시각(KST, 0~23).** 실행 시각이 이 값 이상인 '첫 실행'에서 당일 이슈에 조간뉴스를 발행합니다(하루 1회, 중복 방지). 기본 `7`(오전 7시). 전용 cron(UTC 22:00 = KST 07:00)과 맞물려 매일 07시에 발행됩니다. 이 시각 이전(새벽) 실행에서는 조간을 생성하지 않습니다. |
| `EVENING_REPORT_HOUR` | **Variable** | `21` | **🧠 석간뉴스(당일 요약) 발행 기준 시각(KST, 0~23).** 실행 시각이 이 값 이상이면 당일 이슈에 석간뉴스를 발행합니다(하루 1회, 중복 방지). 기본 `21`(밤 9시). 오프타임 cron(UTC 12:00 = KST 21:00)과 맞물려 매일 21시에 발행됩니다. 실패 시 익일 이슈 종료 시점에 안전망으로 생성됩니다. **주의:** 통합 정리 이메일은 KST 23:00에 발송되며 '석간이 이미 발행됐을 때만' 나갑니다. 이 값을 23 이상으로 올리면 KST 23:00 시점에 석간이 아직 없어 통합 정리 이메일이 건너뛰어질 수 있으니, 그 경우 `consolidated-email.yml`의 cron도 함께 늦춰야 합니다. |
...
| --- | --- | --- | --- |
| 🗓️ 조간뉴스 | `[AI 학습데이터 소송] YYYY-MM-DD 조간 \| 소송 동향 (최근 N일간)` | **KST 07:00** (`MORNING_REPORT_HOUR`, 기본 7시) | `ENABLE_EMAIL_MORNING` |
| 🧠 석간뉴스 | `[AI 학습데이터 소송] YYYY-MM-DD 석간 \| 소송 브리핑 (최근 1일간)` | **KST 21:00** (`EVENING_REPORT_HOUR`, 기본 21시) | `ENABLE_EMAIL_EVENING` |
| 📑 통합 정리 | `[AI 학습데이터 소송] YYYY-MM-DD 당일 소송건들 통합 정리 자료` | **KST 23:00** (`consolidated-email.yml` cron) | `ENABLE_EMAIL_CONSOLIDATED` |
...
| `evening` | `[AI 학습데이터 소송] YYYY-MM-DD 석간 \| 소송 브리핑 (최근 1일간)` | KST 21:00 |
