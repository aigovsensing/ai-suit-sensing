...
| `EVENING_REPORT_HOUR` | **Variable** | `22` | **🧠 석간뉴스(당일 요약) 발행 기준 시각(KST, 0~23).** 실행 시각이 이 값 이상이면 당일 이슈에 석간뉴스를 발행합니다(하루 1회, 중복 방지). 기본 `22`(밤 10시). 전용 cron(UTC 13:00 = KST 22:00)과 맞물려 매일 22시에 발행됩니다. 실패 시 익일 이슈 종료 시점에 안전망으로 생성됩니다. **주의:** 통합 정리 이메일은 KST 23:00에 발송되며 '석간이 이미 발행됐을 때만' 나갑니다. 이 값을 23 이상으로 올리면 KST 23:00 시점에 석간이 아직 없어 통합 정리 이메일이 건너뛰어질 수 있으니, 그 경우 `consolidated-email.yml`의 cron도 함께 늦춰야 합니다. |
...
| 🧠 석간뉴스 | `[AI 학습데이터 소송] YYYY-MM-DD 석간 \| 소송 브리핑 (최근 1일간)` | **KST 22:00** (`EVENING_REPORT_HOUR`, 기본 22시) | `ENABLE_EMAIL_EVENING` |
